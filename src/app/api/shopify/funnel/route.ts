...
export async function GET(request: NextRequest) {
  ...
  // Abandoned checkouts = checkouts started but never placed as an order.
  // Shopify's checkouts/count.json only returns open (incomplete) checkouts.
  const params = new URLSearchParams({ created_at_min, created_at_max });
  ...
  try {
    ...
    if (!res.ok) {
      // checkouts endpoint requires read_checkouts scope; return 0 gracefully
