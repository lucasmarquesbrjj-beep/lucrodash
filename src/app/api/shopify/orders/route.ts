...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const cartaoAprovado = pagos.filter((o: any) => o.payment_gateway?.includes('credit')).length;
    const cartaoPendente = pendentes.filter((o: any) => o.payment_gateway?.includes('credit')).length;
    const boletoPago = pagos.filter((o: any) => o.payment_gateway?.includes('boleto')).length;
    const boletoPendente = pendentes.filter((o: any) => o.payment_gateway?.includes('boleto')).length;
    const pixPago = pagos.filter((o: any) => o.payment_gateway?.includes('pix')).length;
    const pixPendente = pendentes.filter((o: any) => o.payment_gateway?.includes('pix')).length;
